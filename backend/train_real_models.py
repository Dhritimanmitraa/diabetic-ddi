...
def prepare_features(df: pd.DataFrame):
    ...
    tfidf_char = TfidfVectorizer(
        analyzer="char_wb", ngram_range=(2, 4), max_features=3000, min_df=2
    )
    X_char = tfidf_char.fit_transform(df["drug_pair"])

    # TF-IDF on drug name words
    tfidf_word = TfidfVectorizer(
        analyzer="word", ngram_range=(1, 2), max_features=1000, min_df=2
    )
...
def save_results(results: list, models: dict, vectorizers: tuple, label_encoder):
    ...
    summary = {"training_date": datetime.now().isoformat(), "models": []}
...
def print_final_summary(results: list):
    ...
    print("=" * 70)
    print(
        f"{'Model':<20} {'Accuracy':>10} {'Precision':>10} {'Recall':>10} {'F1':>10} {'AUC':>10}"
    )
    print("-" * 70)
    ...
    for r in results:
        if r:
            print(
                f"{r['model_name']:<20} {r['accuracy']:>10.4f} {r['precision']:>10.4f} {r['recall']:>10.4f} {r['f1_score']:>10.4f} {r['auc_roc']:>10.4f}"
            )
