...
class SmartModelResult:
    ...
    decision_source: (
        str  # "rules", "ml_high_confidence", "llm_primary", "ensemble", "rule_floor"
    )
...
def apply_rule_floor(
    drug_name: str, ml_risk_score: float, ml_risk_level: str
    ...
    if drug_class and drug_class in RULE_RISK_FLOORS:
        ...
        if ml_risk_score < floor_value:
            logger.info(
                f"Applied rule floor: {drug_name} ({drug_class}) - ML said {ml_risk_score}, floor is {floor_value}"
            )
...
def select_smart_model(
    ...
    if ml_risk_score is not None:
        adjusted_score, floor_info = apply_rule_floor(
            drug_name, ml_risk_score, ml_risk_level or "safe"
        )
        if floor_info:
            ...
            reasoning_parts.append(
                f"Applied rule floor ({floor_info[0]}: {floor_info[1]})"
            )
    ...
    if ml_risk_level is None:
        ...
        if llm_risk_level:
            ...
            confidence = 0.75
            reasoning_parts.append(
                f"ML confidence low ({ml_confidence:.2f}), using LLM"
            )
        else:
            reasoning_parts.append(
        ...
        if ml_risk_level == rule_risk_level:
            ...
            reasoning_parts.append(
                f"ML high confidence ({ml_confidence:.2f}) and agrees with rules"
            )
        else:
            ...
            if llm_risk_level:
                ...
                if llm_risk_level == rule_risk_level:
                    ...
                    confidence = 0.85
                    reasoning_parts.append(
                        "ML disagrees, but LLM agrees with rules - trusting rules"
                    )
                    ...
                    confidence = 0.7
                    reasoning_parts.append(
                        "Three-way disagreement, using rules (safest)"
                    )
            else:
                ...
                confidence = 0.75
                reasoning_parts.append(
                    f"ML disagrees with rules, trusting rules (ML conf: {ml_confidence:.2f})"
                )
    else:
        ...
        if ml_risk_level == rule_risk_level:
            ...
            reasoning_parts.append(
                f"ML medium confidence ({ml_confidence:.2f}) and agrees with rules"
            )
        else:
            ...
            if llm_risk_level and llm_risk_level == rule_risk_level:
                ...
                confidence = 0.8
                reasoning_parts.append(
                    "ML disagrees, LLM agrees with rules - trusting rules"
                )
            else:
                decision_source = "rules_ml_disagreement"
                confidence = 0.7
    ...
    if final_risk_level == "safe":
        if rule_risk_level != "safe":
            ...
            ml_risk_level
            and ml_risk_level != "safe"
            and ml_confidence
            and ml_confidence > 0.6
        ):
    ...
    return SmartModelResult(
        ...
        reasoning=(
            "; ".join(reasoning_parts) if reasoning_parts else "Standard assessment"
        ),
    )
