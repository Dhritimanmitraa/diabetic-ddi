...
class FeatureAttribution:
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            "value": (
                round(self.feature_value, 3)
                if isinstance(self.feature_value, float)
                else self.feature_value
            ),
...
class SHAPExplainer:
    ...
    def explain_prediction(
        self, feature_vector: np.ndarray, patient_context: Dict, top_n: int = 5
            ...
            if isinstance(shap_values, list):
                ...
                try:
                    if hasattr(self.model, "predict_proba"):
                        ...
                        pred_class = (
                            int(pred) if isinstance(pred, (int, np.integer)) else 0
                        )
                    ...
                    pred_class = (
                        min(pred_class, len(shap_values) - 1) if pred_class >= 0 else 0
                    )
                ...
                base_value = (
                    ...
                    if pred_class < len(self.explainer.expected_value)
    ...
    def _generate_explanation_text(
        self, top_factors: List[FeatureAttribution], patient_context: Dict
    ) -> str:
