...
class DiabeticMLPredictorV2:
    ...
    def _get_drug_class_features(self, drug_name: str) -> List[float]:
        ...
        hypoglycemia = ["glipizide", "glyburide", "glimepiride", "insulin", "sulfonylurea", "quinine"]
        if any(h in drug_lower for h in hypoglycemia):
            features[0] = 1
        
        # Hyperglycemia risk (corticosteroids, antipsychotics, etc.)
        hyperglycemia = ["prednisone", "dexamethasone", "hydrocortisone", "methylprednisolone",
        ...
        # Nephrotoxic
        nephrotoxic = ["ibuprofen", "naproxen", "diclofenac", "celecoxib", "meloxicam",
                       "gentamicin", "tobramycin", "amikacin", "vancomycin",
                       "amphotericin", "acyclovir", "tenofovir", "cisplatin", "methotrexate"]
        if any(n in drug_lower for n in nephrotoxic):
            features[2] = 1
        
        # Hyperkalemia risk
        hyperkalemia = ["spironolactone", "eplerenone", "amiloride", "trimethoprim",
                        "pril", "sartan"]  # ACE/ARB
        if any(h in drug_lower for h in hyperkalemia):
            features[3] = 1
        
        # Hepatotoxic
        hepatotoxic = ["acetaminophen", "amiodarone", "valproic", "isoniazid", "methotrexate"]
        if any(h in drug_lower for h in hepatotoxic):
            features[4] = 1
        
        # Cardioprotective
        cardioprotective = ["empagliflozin", "canagliflozin", "dapagliflozin",
        ...
        # Is corticosteroid
        if any(c in drug_lower for c in ["prednisone", "dexamethasone", "hydrocortisone", "methylpred"]):
            features[7] = 1
        
        # Is NSAID
        if any(n in drug_lower for n in ["ibuprofen", "naproxen", "diclofenac", "celecoxib", "meloxicam"]):
            features[8] = 1
        ...
        # Is diuretic
        if any(d in drug_lower for d in ["furosemide", "hydrochlorothiazide", "chlorthalidone", "bumetanide"]):
            features[10] = 1
        ...
        # Is sulfonylurea
        if any(s in drug_lower for s in ["glipizide", "glyburide", "glimepiride", "gliclazide"]):
            features[12] = 1
    ...
    def predict(self, drug_name: str, patient: Dict) -> Optional[DiabeticMLResultV2]:
        ...
        try:
            ...
            prob_map = {self.classes[i]: float(proba[i]) for i in range(len(self.classes))}
