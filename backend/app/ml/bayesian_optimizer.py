...
class OptimizationResult:
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            "trials_per_second": (
                self.n_trials / self.total_time if self.total_time > 0 else 0
            ),
        }
...
class BayesianOptimizer:
    ...
    def _get_param_suggestions(self, trial: optuna.Trial, param_space: Dict) -> Dict:
    ...
    def _create_objective(
        self, X: np.ndarray, y: np.ndarray, param_space: Dict
    ) -> Callable:
            ...
            try:
                scores = cross_val_score(
                    (
                        model.model
                        if model.model
                        else DDIModelFactory.create(self.model_type, params).model
                    ),
                    X,
                    y,
                    cv=cv,
                ...
                if model.model is None:
                    ...
                    scores = cross_val_score(
                        temp_model.model, X, y, cv=cv, scoring=self.scoring, n_jobs=-1
                    )
...
class OptimizationComparator:
    ...
    def run_comparison(
        self, X: np.ndarray, y: np.ndarray, param_space: Optional[Dict] = None
        ...
        if OptimizationMethod.BAYESIAN in self.results:
            ...
            if random and random.n_trials > 0:
                summary["efficiency_gain"] = {
                    ...
                    "trial_reduction_percent": (1 - bayesian.n_trials / random.n_trials)
                    * 100,
                    "time_reduction_percent": (
                        (1 - bayesian.total_time / random.total_time) * 100
                        if random.total_time > 0
                        else 0
                    ),
                }
