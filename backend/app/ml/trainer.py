...
from app.ml.bayesian_optimizer import (
    BayesianOptimizer, OptimizationComparator, 
...
class DDITrainer:
    ...
    def load_data_from_dicts(
        ...
        """
        logger.info(f"Loading data: {len(drugs)} drugs, {len(interactions)} interactions")
    ...
    def train_single_model(
        ...
        if optimize:
            if run_comparison:
                ...
                self.comparison_results[model_type] = comparator.get_comparison_summary()
                
                # Use Bayesian result
                self.optimization_results[model_type] = results[OptimizationMethod.BAYESIAN]
...
async def train_from_database(
    ...
    result = await db_session.execute(
        select(DrugInteraction).options()
    )
