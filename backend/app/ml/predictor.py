        ...
        try:
            ...
            if os.path.exists(results_path):
                ...
                for model_name in self.models:
                    if model_name in self.model_info.get('models', {}):
                        metrics = self.model_info['models'][model_name].get('metrics', {})
            ...
            if os.path.exists(threshold_path):
                with open(threshold_path, 'r') as f:
                    ...
                    logger.info(f"Loaded optimal threshold: {self.optimal_threshold:.4f} (method: {self.threshold_method})")
            else:
                logger.warning(f"No optimal threshold found, using default: {self.DEFAULT_THRESHOLD}")
            
            self.is_loaded = True
            logger.info(f"Predictor loaded with {loaded_count} models (simple_features={self.use_simple_features}, threshold={self.optimal_threshold:.4f})")
            return True
