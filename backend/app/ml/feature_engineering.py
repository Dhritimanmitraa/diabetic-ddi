...
class DrugFeatureExtractor:
    ...
    def __init__(self):
        ...
        self.mechanism_vectorizer = TfidfVectorizer(max_features=100, stop_words='english')
        self.indication_vectorizer = TfidfVectorizer(max_features=100, stop_words='english')
    ...
    def fit(self, drugs: List[Dict], interactions: List[Dict]):
        ...
        self.is_fitted = True
        logger.info(f"Feature extractor fitted on {len(drugs)} drugs and {len(interactions)} interactions")
...
def prepare_training_data(
    ...
    logger.info(f"Prepared {len(positive_pairs)} positive and {len(negative_pairs)} negative samples")
