...
class Drug(Base):
    ...
    interactions_as_drug1 = relationship("DrugInteraction", foreign_keys="DrugInteraction.drug1_id", back_populates="drug1")
    interactions_as_drug2 = relationship("DrugInteraction", foreign_keys="DrugInteraction.drug2_id", back_populates="drug2")
...
class DrugInteraction(Base):
    ...
    evidence_level = Column(String(50), nullable=True)  # established, theoretical, case_report
...
class DrugSimilarity(Base):
    ...
    therapeutic_similarity = Column(Float, default=0.0)  # Based on drug class/indication
...
class ComparisonLog(Base):
    ...
    ml_decision_source = Column(String(50), nullable=True)  # ml_primary, rule_override, rules_only
    ml_model_version = Column(String(100), nullable=True)  # Model version used
    rule_override_reason = Column(Text, nullable=True)  # Why rule overrode ML (if applicable)
    ...
    def __repr__(self):
        has_interaction_val = self.has_interaction
        status = "INTERACTION" if has_interaction_val else "SAFE"  # type: ignore[truthy-bool]
...
class ModelMetrics(Base):
    ...
    model_type = Column(String(50), nullable=False, index=True)  # random_forest, xgboost, lightgbm
...
class OptimizationResult(Base):
    ...
    optimization_method = Column(String(50), nullable=False)  # bayesian, grid_search, random_search
...
class TwosidesInteraction(Base):
    ...
    severity = Column(String(50), nullable=True, index=True)  # mapped severity (fatal/contra/major/moderate/minor)
...
class OffsidesEffect(Base):
    ...
    effect = Column(Text, nullable=True)
    severity = Column(String(50), nullable=True, index=True)  # optional mapped severity if we choose to classify
