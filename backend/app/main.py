...
async def log_drug_requests(request: Request, call_next):
    ...
    if path.startswith("/drugs"):
        ...
        print(
            f"[search-log] {request.method} {path} query={info['query']} ip={client_ip}"
        )
...
async def check_interaction(
    request: InteractionCheckRequest, req: Request, db: AsyncSession = Depends(get_db)
):
    """
    ...
    if result.interaction and result.interaction.severity in [
        ...
        rule_override_reason = (
            f"Rule flagged {result.interaction.severity} severity interaction"
        )
...
@app.get(
    "/interactions/check/{drug1}/{drug2}",
    response_model=InteractionCheckResponse,
    tags=["Interactions"],
)
async def check_interaction_get(
    drug1: str, drug2: str, req: Request, db: AsyncSession = Depends(get_db)
):
    """
...
async def get_drug_interactions(
    drug_name: str, severity: Optional[str] = None, db: AsyncSession = Depends(get_db)
):
    """
        ...
        "interactions": [
            {
                ...
                "other_drug": (
                    i.drug2.name
                    if i.drug1.name.upper() == drug_name.upper()
                    else i.drug1.name
                ),
                "severity": i.severity,
                "effect": i.effect,
...
@app.post(
    "/alternatives", response_model=AlternativeSuggestionResponse, tags=["Alternatives"]
)
async def get_alternatives(
    request: InteractionCheckRequest, db: AsyncSession = Depends(get_db)
):
    """
    ...
    try:
        return await service.find_alternatives(request.drug1_name, request.drug2_name)
...
@app.get(
    ...
    response_model=AlternativeSuggestionResponse,
    tags=["Alternatives"],
)
async def get_alternatives_get(
    drug1: str, drug2: str, db: AsyncSession = Depends(get_db)
):
...
async def extract_from_image(request: OCRRequest, db: AsyncSession = Depends(get_db)):
    """
    ...
    try:
        ...
        logger.info(
            f"Attempting to match {len(detected_drugs)} detected drug names with database"
        )
            ...
            if drugs:
                ...
                if all_drug_names:
                    fuzzy_matches = ocr_service.find_similar_drug_names(
                        drug_name,
                        all_drug_names,
                        threshold=0.5,  # Lower threshold for OCR errors
                    )
                    if fuzzy_matches:
                        matched_drugs.append(fuzzy_matches[0][0])
                        logger.info(
                            f"Fuzzy matched '{drug_name}' -> '{fuzzy_matches[0][0]}' (score: {fuzzy_matches[0][1]:.2f})"
                        )
                    else:
...
async def get_dangerous_combinations(db: AsyncSession = Depends(get_db)):
    ...
    return {"total_dangerous_checks": len(dangerous), "combinations": dangerous}


@app.get("/history/popular-drugs", tags=["History"])
async def get_most_checked_drugs(limit: int = 10, db: AsyncSession = Depends(get_db)):
    """
    ...
    return {"most_checked_drugs": popular}
...
async def get_optimization_comparison():
        ...
        if os.path.exists(filepath):
            with open(filepath, "r") as f:
                comparison_files.append(
                    {"model": model_type, "comparison": json.load(f)}
                )
    ...
    return {
        ...
        "average_trial_reduction_percent": (
            sum(avg_efficiency_gain) / len(avg_efficiency_gain)
            if avg_efficiency_gain
            else 0
        ),
...
async def trigger_model_training(
    ...
    """
    await rate_limit(
        request, limit=settings.RATE_LIMIT_REQUESTS_PER_MIN, key_prefix="ml-train"
    )
...
async def enqueue_data_fetch(
    ...
    await rate_limit(
        request, limit=settings.RATE_LIMIT_REQUESTS_PER_MIN, key_prefix="data-refresh"
    )
