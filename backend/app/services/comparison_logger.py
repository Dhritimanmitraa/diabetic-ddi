...
class ComparisonLogger:
    ...
    async def log_comparison(
        ...
        logger.info(f"Logged comparison: {drug1_name} + {drug2_name} = {'INTERACTION' if has_interaction else 'SAFE'}{decision_info}")
    ...
    async def _append_to_json_log(self, log_entry: ComparisonLog):
        ...
        try:
            ...
            entry = {
                ...
                "drug1": {
                    "name": log_entry.drug1_name,
                    "id": log_entry.drug1_id
                },
                "drug2": {
                    "name": log_entry.drug2_name,
                    "id": log_entry.drug2_id
                },
                "result": {
    ...
    async def get_comparisons(
        ...
        base_query = base_query.order_by(desc(ComparisonLog.timestamp)).offset(offset).limit(limit)
    ...
    async def get_most_checked_drugs(self, limit: int = 10) -> List[Dict]:
        ...
        result = await self.db.execute(
            select(
                ComparisonLog.drug1_name,
                func.count(ComparisonLog.id).label('count')
            )
