...
class InteractionService:
    ...
    SEVERITY_RANKING = {"contraindicated": 4, "major": 3, "moderate": 2, "minor": 1}
    ...
    async def check_interaction(
        ...
        if interaction:
            ...
            return InteractionCheckResponse(
                ...
                safety_message=safety_messages.get(
                    severity, "Unknown interaction severity."
                ),
    ...
    async def _get_interaction(
        self, drug1_id: int, drug2_id: int
        ...
        stmt = (
            ...
            .options(
                selectinload(DrugInteraction.drug1), selectinload(DrugInteraction.drug2)
            )
        )
    ...
    def _create_unknown_drug_response(
        self, drug_name: str, position: str
    ) -> InteractionCheckResponse:
        """Create response for unknown drug."""
        from datetime import datetime

        unknown_drug = DrugResponse(
            id=0, name=drug_name, is_approved=False, created_at=datetime.utcnow()
        )

        placeholder_drug = DrugResponse(
            id=0, name="Unknown", is_approved=False, created_at=datetime.utcnow()
        )
    ...
    async def find_alternatives(
        self, drug1_name: str, drug2_name: str, max_alternatives: int = 5
    ) -> AlternativeSuggestionResponse:
        """
        ...
        alternatives_for_drug1 = await self._find_safe_alternatives(
            drug1, drug2, max_alternatives
        )

        # Find alternatives for drug2 (similar drugs that don't interact with drug1)
        alternatives_for_drug2 = await self._find_safe_alternatives(
            drug2, drug1, max_alternatives
        )

        # Find safe combinations
        safe_combinations = await self._find_safe_combinations(
            alternatives_for_drug1, alternatives_for_drug2
        )
    ...
    async def _find_safe_alternatives(
        self, target_drug: Drug, other_drug: Drug, max_alternatives: int
        ...
        alternatives.sort(
            key=lambda x: (-int(not x.has_interaction_with_other), -x.similarity_score)
        )

        return alternatives[:max_alternatives]

    async def _get_similar_drugs(
        self, drug: Drug, limit: int = 20
        ...
        if drug.drug_class:
            stmt = (
                select(Drug)
                .where(and_(Drug.drug_class == drug.drug_class, Drug.id != drug.id))
                .limit(limit)
            )
        ...
        stmt = (
            ...
            .order_by(DrugSimilarity.overall_similarity.desc())
            .limit(limit)
        )
    ...
    def _calculate_drug_similarity(self, drug1: Drug, drug2: Drug) -> float:
        ...
        if drug1.indication and drug2.indication:
            indication_sim = SequenceMatcher(
                None, drug1.indication.lower()[:200], drug2.indication.lower()[:200]
            ).ratio()
        ...
        if drug1.mechanism and drug2.mechanism:
            mechanism_sim = SequenceMatcher(
                None, drug1.mechanism.lower()[:200], drug2.mechanism.lower()[:200]
            ).ratio()
            score += 0.2 * mechanism_sim
            weight_sum += 0.2

        # Name similarity (weight: 0.1)
        name_sim = SequenceMatcher(None, drug1.name.lower(), drug2.name.lower()).ratio()
        score += 0.1 * name_sim
        weight_sum += 0.1
    ...
    async def _find_safe_combinations(
        self, alternatives1: List[AlternativeDrug], alternatives2: List[AlternativeDrug]
    ) -> List[Dict]:
                    ...
                    safe_combinations.append(
                        {
                            ...
                            "combined_similarity": (
                                alt1.similarity_score + alt2.similarity_score
                            )
                            / 2,
    ...
    async def get_all_interactions_for_drug(
        self, drug_name: str, severity_filter: Optional[str] = None
