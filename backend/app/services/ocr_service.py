...
class DrugOCRService:
    ...
    def _extract_drug_names(self, text: str) -> List[str]:
        ...
        for pattern_idx, pattern in enumerate(self.DRUG_PATTERNS):
            ...
            for match in matches:
                ...
                cleaned = (
                    cleaned.replace(" ER", "").replace(" SR", "").replace(" XR", "")
                )
        ...
        for word in words:
            ...
            if (
                cleaned.isupper()
                and 3 <= len(cleaned) <= 12
                and cleaned.replace("-", "").replace("&", "").isalnum()
    ...
    def find_similar_drug_names(
        self, detected_name: str, known_drugs: List[str], threshold: float = 0.6
        ...
        for drug in known_drugs:
            ...
            if detected_upper in drug_upper or drug_upper in detected_upper:
                score = len(min(detected_upper, drug_upper)) / len(
                    max(detected_upper, drug_upper)
                )
                if score >= threshold:
