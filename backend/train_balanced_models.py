...
def evaluate_model(model, X_test, y_test, model_name: str, label_encoder):
    ...
    X_eval = (
        X_test.toarray()
        if hasattr(X_test, "toarray") and "Gradient" in model_name
        else X_test
    )
...
def print_final_summary(results):
    ...
    for r in results:
        if r:
            print(
                f"{r['model_name']:<25} {r['accuracy']:>10.4f} {r['precision']:>10.4f} {r['recall']:>10.4f} {r['f1_score']:>10.4f} {r['auc_roc']:>10.4f}"
            )
