...
def evaluate_at_multiple_thresholds(
    y_true, y_proba, thresholds=[0.3, 0.4, 0.5, 0.6, 0.7]
):
    ...
    for metric in [
        ...
        if metric in saved_metrics and metric in current_metrics:
            ...
            if metric == "brier_score":
                ...
                print(
                    f"{metric:<20} {saved_val:>15.6f} {curr_val:>15.6f} {diff:>15.6f}"
                )
            else:
                print(
                    f"{metric:<20} {saved_val*100:>14.2f}% {curr_val*100:>14.2f}% {diff*100:>14.2f}%"
                )
...
def main():
    ...
    parser.add_argument(
        "--compare", action="store_true", help="Compare with saved training metrics"
    )
    parser.add_argument(
        "--find-optimal", action="store_true", help="Find and save optimal threshold"
    )
    parser.add_argument(
        "--threshold-method",
        type=str,
    ...
    for model_name, model in models.items():
        metrics, probas = evaluate_model(
            model, X_test, y_test, model_name, args.threshold
        )
    ...
    print("=" * 80)
    print(
        f"{'Model':<20} {'Accuracy':>12} {'F1-Score':>12} {'AUC-ROC':>12} {'Precision':>12} {'Recall':>12}"
    )
    print("─" * 80)
    ...
    for model_name in ["random_forest", "xgboost", "lightgbm"]:
        if model_name in all_metrics:
            ...
            print(
                f"{model_name:<20} {m['accuracy']*100:>11.2f}% {m['f1_score']*100:>11.2f}% "
    ...
    if len(models) > 1:
        e = ensemble_metrics
        print(
            f"{'ENSEMBLE':<20} {e['accuracy']*100:>11.2f}% {e['f1_score']*100:>11.2f}% "
    ...
    if args.find_optimal:
        ...
        if len(models) > 1:
            ...
            best_model = max(
                all_metrics.keys(), key=lambda k: all_metrics[k]["auc_roc"]
            )
        ...
        multi_results = evaluate_at_multiple_thresholds(
            y_test, analysis_proba, test_thresholds
        )

        for r in multi_results:
            marker = " ←OPTIMAL" if abs(r["threshold"] - opt_thresh) < 0.001 else ""
            print(
                f"{r['threshold']:>10.4f} {r['accuracy']*100:>9.2f}% {r['precision']*100:>9.2f}% "
        ...
        default_metrics = all_metrics.get(
            "random_forest", list(all_metrics.values())[0]
        )
        ...
        for metric in [
            ...
            if metric in default_metrics and metric in opt_metrics:
                ...
                print(
                    f"{metric:<20} {default_val*100:>14.2f}% {opt_val*100:>14.2f}% "
                    f"{arrow} {abs(change)*100:>12.2f}%"
                )

        # False negatives comparison
        default_fn = default_metrics.get("confusion_matrix", {}).get(
            "false_negatives", 0
        )
        opt_fn = opt_metrics["confusion_matrix"]["false_negatives"]
        fn_reduction = default_fn - opt_fn
        print(
            f"{'False Negatives':<20} {default_fn:>15,} {opt_fn:>15,} "
