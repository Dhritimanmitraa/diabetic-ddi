...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train ML models on TWOSIDES data with imbalanced-learn"
    )
    parser.add_argument(
        "--max-samples",
        type=int,
    ...
    parser.add_argument(
        "--fast", action="store_true", help="Fast mode: fewer trials, simpler models"
    )
    parser.add_argument("--skip-xgb", action="store_true", help="Skip XGBoost training")
    parser.add_argument(
        "--skip-lgb", action="store_true", help="Skip LightGBM training"
    )
    parser.add_argument(
        "--n-jobs",
        type=int,
...
def load_training_data_optimized(data_dir: Path, max_samples: int = None):
    ...
    if max_samples and total_train > max_samples:
        ...
        skip_indices = [
            i + 1 for i, skip in enumerate(skip_rows) if skip
        ]  # +1 for header
...
def train_xgboost(X_train, y_train, X_val, y_val, n_jobs=-1, fast=False):
    ...
    elapsed = time.time() - start
    logger.info(
        f"XGBoost trained in {elapsed:.1f}s (stopped at {model.best_iteration} trees)"
    )
...
def train_lightgbm(X_train, y_train, X_val, y_val, n_jobs=-1, fast=False):
    ...
    elapsed = time.time() - start
    logger.info(
        f"LightGBM trained in {elapsed:.1f}s (stopped at {model.best_iteration_} trees)"
    )
...
def evaluate_model(model, X_test, y_test, model_name, threshold=0.5):
    ...
    metrics = {
        ...
        ),  # Negative Predictive Value (CRITICAL!)
        "confusion_matrix": {
            "tn": int(tn),
            "fp": int(fp),
            "fn": int(fn),
...
def main():
    ...
    train_df, val_df, test_df = load_training_data_optimized(
        data_dir, max_samples=args.max_samples
    )
    ...
    logger.info(
        f"Train distribution BEFORE resampling: 0={np.sum(y_train==0)}, 1={np.sum(y_train==1)}"
    )
    ...
    if args.resampling != "none":
        ...
        try:
            ...
            logger.info(
                f"Train distribution AFTER resampling: 0={np.sum(y_train_resampled==0)}, 1={np.sum(y_train_resampled==1)}"
            )
            logger.info(
                f"New class balance ratio: {np.sum(y_train_resampled==1) / np.sum(y_train_resampled==0):.4f}"
            )
    ...
    rf_model, rf_params = train_random_forest(
        X_train, y_train, X_val, y_val, n_jobs=args.n_jobs, fast=args.fast
    )
    ...
    if len(models) > 1:
        ...
        ensemble_metrics = {
            ...
            "npv": (
                tn_ens / (tn_ens + fn_ens) if (tn_ens + fn_ens) > 0 else 0
            ),  # CRITICAL!
            "confusion_matrix": {
                "tn": int(tn_ens),
                "fp": int(fp_ens),
        ...
        logger.info(
            f"  Confusion Matrix: TN={tn_ens}, FP={fp_ens}, FN={fn_ens}, TP={tp_ens}"
        )
    ...
    print("-" * 60)
    print(
        f"{'Model':<15} {'AUC':>8} {'F1':>8} {'NPV':>8} {'Recall':>8} {'Precision':>10}"
    )
    print("-" * 60)
    ...
    for model_name, model_data in results["models"].items():
        ...
        print(
            f"{model_name:<15} {m['auc_roc']:>8.4f} {m['f1_score']:>8.4f} {m.get('npv', 0):>8.4f} {m.get('recall', 0):>8.4f} {m.get('precision', 0):>10.4f}"
        )
    ...
    if "ensemble" in results:
        ...
        print("-" * 60)
        print(
            f"{'ENSEMBLE':<15} {e['auc_roc']:>8.4f} {e['f1_score']:>8.4f} {e.get('npv', 0):>8.4f} {e.get('recall', 0):>8.4f} {e.get('precision', 0):>10.4f}"
        )
        print("-" * 60)
