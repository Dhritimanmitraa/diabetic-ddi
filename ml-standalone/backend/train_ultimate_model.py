...
HIGH_RISK_INTERACTIONS = {
    ...
    ("cyp3a4_inhibitor", "cyp3a4_substrate"): {
        "severity": 2,
        "reason": "increased_levels",
    },
    ("cyp_inducer", "narrow_therapeutic"): {
        "severity": 3,
        "reason": "decreased_efficacy",
    },
    ("cyp_inhibitor", "narrow_therapeutic"): {"severity": 3, "reason": "toxicity_risk"},
    ("potassium_raising", "potassium_raising"): {
        "severity": 2,
        "reason": "hyperkalemia",
    },
    ("potassium_depleting", "cardiac_glycoside"): {
        "severity": 3,
        "reason": "digoxin_toxicity",
    },
    ("nephrotoxic", "nephrotoxic"): {"severity": 2, "reason": "kidney_damage"},
    ("hepatotoxic", "hepatotoxic"): {"severity": 2, "reason": "liver_damage"},
    ("anticholinergic", "anticholinergic"): {
        "severity": 2,
        "reason": "anticholinergic_burden",
    },
...
def get_interaction_features(classes1: list, classes2: list) -> dict:
    ...
    for (class1, class2), risk_info in HIGH_RISK_INTERACTIONS.items():
        if (
            (class1 in classes1 and class2 in classes2)
            or (class2 in classes1 and class1 in classes2)
            or (class1 in all_classes and class2 in all_classes)
        ):
...
def train_ultimate_model():
    ...
    for d1, d2, _ in interactions:
        if d1:
            all_drugs.add(d1.lower().strip())
        if d2:
            all_drugs.add(d2.lower().strip())
    ...
    for _ in range(neg_count * 3):
        ...
        if d1 != d2:
            ...
            if pair not in interaction_set:
                X_text.append(pair)
                classes1, classes2 = get_drug_classes(drugs[0]), get_drug_classes(
                    drugs[1]
                )
                X_interaction_features.append(
                    list(get_interaction_features(classes1, classes2).values())
                )
                y.append(0)
    ...
    tfidf = TfidfVectorizer(
        analyzer="char_wb", ngram_range=(2, 5), max_features=6000, min_df=2
    )
    X_tfidf = tfidf.fit_transform(X_text)

    word_vec = TfidfVectorizer(
        analyzer="word", ngram_range=(1, 2), max_features=2000, min_df=2
    )
    ...
    logger.info(f"\n{'='*60}")
    logger.info(
        f"ULTIMATE MODEL ACCURACY: {final_accuracy:.4f} ({final_accuracy*100:.2f}%)"
    )
    ...
    print(
        classification_report(
            y_test,
            y_pred,
            target_names=["none", "mild", "moderate", "severe", "contraindicated"][
                : len(set(y))
            ],
        )
    )
