    ...
    def load_model(self) -> bool:
        ...
        try:
            ...
            if not model_path.exists():
                logger.warning(
                    f"Model not found at {model_path}. Run train_advanced_model.py first."
                )
                return False
            ...
            self.is_loaded = True
            logger.info(
                f"ML Model v{self.model_info.get('version', '?')} loaded. Accuracy: {self.model_info.get('accuracy', 'N/A'):.4f}"
            )
            return True
