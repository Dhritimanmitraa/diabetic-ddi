...
async def lifespan(app: FastAPI):
    ...
    logger.info(
        f"Prediction cache ready with {len(prediction_cache.cache)} cached predictions"
    )
    ...
    try:
        ...
        if ml_model.is_loaded:
            logger.info(
                f"ML Model ready - Version: {ml_model.model_info.get('version', 'N/A')}"
            )
        else:
            logger.warning(
                "ML Model not loaded. Run train_model.py to train the model."
            )
    ...
    try:
        ...
        logger.info(
            f"Validation Service ready with {gt_count} ground truth interactions"
        )
    except Exception as e:
...
async def health_check():
    ...
    # Validation stats
    validation_enabled = (
        validation_service is not None and len(validation_service.ground_truth) > 0
    )
    ...
    status = (
        "healthy"
        if ml_model_loaded
        else ("degraded" if ollama_connected else "unhealthy")
    )
...
@app.post("/predict", response_model=DrugPredictionResponse, tags=["Prediction"])
    ...
    **Note**: Works with just ML model if Ollama is not available.
    ...
    if request.use_cache and prediction_cache:
        ...
        if cached:
            ...
            return DrugPredictionResponse(
                ...
                ml_prediction=(
                    MLPrediction(**cached_prediction["ml_prediction"])
                    if cached_prediction.get("ml_prediction")
                    else None
                ),
                validation=(
                    ValidationResult(**cached_prediction["validation"])
                    if cached_prediction.get("validation")
                    else None
                ),
    ...
    if validation_service and ml_prediction_result:
        ...
        calibrated_confidence = validation_service.calibrate_confidence(
            ml_prediction_result["confidence"], validation_result
        )
        ...
        validation_schema = ValidationResult(
            ...
            ground_truth_severity=(
                validation_result.get("ground_truth", {}).get("severity")
                if validation_result.get("ground_truth")
                else None
            ),
    ...
    if llm_service:
        try:
            ...
            enhanced_context = (
                twosides_context_dict.copy() if twosides_context_dict else {}
            )
    ...
    if ml_prediction_result and llm_prediction:
        ...
        confidence = (
            validation_schema.calibrated_confidence
            if validation_schema
            else ml_prediction_result["confidence"]
        )
    ...
    if request.use_cache and prediction_cache and not is_cached:
        cache_data = {
            ...
            "ml_prediction": (
                ml_prediction_schema.model_dump() if ml_prediction_schema else None
            ),
...
@app.get("/predict/ml", response_model=MLPrediction, tags=["Prediction"])
...
async def search_drugs(
    ...
    if not data_service or not data_service.is_connected:
        return DrugSearchResponse(query=query, results=[], total_count=0)

    results = await data_service.search_drugs(query, limit)

    return DrugSearchResponse(query=query, results=results, total_count=len(results))


@app.get("/models", tags=["Models"])
async def list_models():
    """List available models (ML and LLM)"""
    response = {"ml_model": None, "llm_models": [], "current_llm": None}
