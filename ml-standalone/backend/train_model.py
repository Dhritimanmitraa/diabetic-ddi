...
def train_model():
    ...
    try:
        ...
        logger.error(
            "Install with: pip install scikit-learn xgboost joblib numpy pandas"
        )
        return False
    ...
    if not db_path:
        logger.warning(
            "TWOSIDES database not found. Creating synthetic training data..."
        )
    ...
    logger.info(
        f"Total samples: {len(X_text)} (positive: {sum(1 for l in y if l > 0)}, negative: {sum(1 for l in y if l == 0)})"
    )
